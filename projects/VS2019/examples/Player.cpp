...
void Player::DetectWalls(std::vector<ModelCube*>& columns)
{
	...
	for (ModelCube* column : columns)
	{
		...
		{
			//Set the camera in the contact point
			m_Camera->position = info.position;
...
void Player::Update(float deltaTime, std::vector<ModelCube*>& columns)
{
